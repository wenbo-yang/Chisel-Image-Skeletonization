...
export function zsThinnigGetTargetPointsStep1(mat: number[][]): Point[] {
...
export function isThisATargetToRemoveStep1(mat: number[][], r: number, c: number, neighbors: number[]): boolean {
    if (mat[r][c] === 1) {
        const sumInRange = sumOfNeighborsInRemovalRange(neighbors);
        const numTransIsOne = numberOfTransitionsIsInRemovalRange(neighbors);
...
export function isNotOnTheEdge(mat: number[][], i: number, j: number) {
    const row = mat.length;
    const col = mat[0].length;
...
export function sumOfNeighborsInRemovalRange(neighbors: number[]): boolean {
    let sum = 0;
...
export function isAtLeastOneNumberBlankStep1(neighbors: number[]): boolean {
...
export function isAtLeastOneNumberBlankStep2(neighbors: number[]): boolean {
...
export function removePoints(mat: number[][], pointsToRemove: Point[]): void {
