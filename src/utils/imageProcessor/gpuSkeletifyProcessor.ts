...
export class GpuSkeletifyProcessor implements SkeletifyProcessor {
    ...
    public async thinning(bitMapBuffer: BitMapBuffer): Promise<Array<number[]>> {
        ...
        const thinning = (new GPU()).createKernel(function(mat: number[][], removalMat: number[][], neigbors: number[]) {   
            function zsThinnigGetTargetPointsStep1(mat: number[][], removalMat: number[][], row: number, col: number): boolean {
            ...
            function zsThinnigGetTargetPointsStep2(mat: number[][], removalMat: number[][], row: number, col: number): boolean {
                let hasPointsToRemove = false;
                for (let i = 0; i < row; i++) {
                    for (let j = 0; j < col; j++) {
            ...
            do {
                if (shouldRunStep1) {
                    hasPointsToRemove = zsThinnigGetTargetPointsStep1(mat, removalMat, row, col);
                    removePoints(mat, removalMat, row, col);
                } else {
                    hasPointsToRemove = zsThinnigGetTargetPointsStep2(mat, removalMat, row, col);
                    removePoints(mat, removalMat, row, col);
                }
